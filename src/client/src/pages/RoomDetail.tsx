...
import { socket } from "../services/socket"; // Імпортуємо сокет
...
export const RoomDetail = () => {
  ...
  useEffect(() => {
    ...
    if (roomId && parsedUser) {
      ...
      socket.on('participants_updated', (updatedParticipants: any[]) => {
  ...
  if (!user) return <div style={{ color: '#333', padding: '20px' }}>Увійдіть, щоб переглядати кімнату.</div>;

  return (
    <div style={{ backgroundColor: '#f5f5f5', color: '#333', flexGrow: 1, padding: '20px', display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <NavBar />
      <h2 style={{ borderBottom: '2px solid #ddd', paddingBottom: '10px', color: '#222' }}>
        Кімната: {roomName}
      </h2>

      <div style={{ display: 'flex', flexGrow: 1, gap: '20px', marginTop: '10px' }}>
        ...
        <div style={{ width: '250px', backgroundColor: '#ffffff', border: '1px solid #ddd', borderRadius: '8px', padding: '15px', display: 'flex', flexDirection: 'column' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, overflowY: 'auto' }}>
            {participants.length === 0 ? (
              <li style={{ color: '#888', fontSize: '14px' }}>Немає учасників</li>
            ) : (
              participants.map((p: any) => (
                <li key={p._id || p.id || p} style={{ padding: '8px 0', borderBottom: '1px solid #f9f9f9', display: 'flex', alignItems: 'center' }}>
                  <span style={{ display: 'inline-block', width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#28a745', marginRight: '8px' }}></span>
                  {p.userName || p.name || 'Невідомий'}
                </li>
              ))
            )}
          </ul>
